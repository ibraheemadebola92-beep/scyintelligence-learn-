...
def call_ai(messages, system_prompt):
    ...
    for model in MODELS:
        try:
            payload = {"model": model, "messages": [{"role": "system", "content": system_prompt}] + messages, "max_tokens": 4000}
...
# ══════════════════════════════════════
    ...
    if st.button("Teach Me 🚀"):
        ...
        system = f"You are an expert university tutor for 100-Level Engineering students in Nigeria. Always write mathematical equations using $ for inline math and $$ for display math. Never use raw LaTeX without delimiters. Course: {course}. Topic: {topic}. Teach clearly with: 1. Simple explanation 2. Detailed breakdown 3. 2 worked examples 4. Exam tips."
...
No markdown, no preamble. Only the JSON array."""
        ...
        if q["type"] == "mcq":
            ...
            option_list = ["-- Select an answer --"] + list(options.keys())
            choice = st.radio("Select your answer:", option_list, format_func=lambda x: x if x == "-- Select an answer --" else f"{x}. {options[x]}", key=f"cbt_mcq_{current}")
