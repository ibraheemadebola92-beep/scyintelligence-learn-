...
.question-box { background: rgba(0,255,136,0.05); border-left: 3px solid #00ff88; border-radius: 8px; padding: 16px; margin: 12px 0; }
.result-correct { background: rgba(0,255,136,0.1); border-left: 3px solid #00ff88; border-radius: 8px; padding: 12px; margin: 8px 0; }
.result-wrong { background: rgba(255,60,60,0.1); border-left: 3px solid #ff3c3c; border-radius: 8px; padding: 12px; margin: 8px 0; }
.score-box { background: linear-gradient(135deg, #00ff88, #00cc6a); color: #0a0a0f; border-radius: 16px; padding: 24px; text-align: center; font-size: 2rem; font-weight: 800; }
.chat-user { background: rgba(0,255,136,0.08); border-radius: 12px 12px 2px 12px; padding: 12px 16px; margin: 8px 0; text-align: right; }
.chat-ai { background: rgba(255,255,255,0.05); border-radius: 12px 12px 12px 2px; padding: 12px 16px; margin: 8px 0; }
...
.stButton > button { background: linear-gradient(135deg, #00ff88, #00cc6a); color: #0a0a0f; font-weight: 700; border: none; border-radius: 10px; padding: 12px 28px; width: 100%; }
...
with st.sidebar:
    ...
    st.markdown("---")
    mode = st.radio("Select Mode", ["📚 Study Mode", "✍️ Practice Mode", "🖥️ CBT Mode", "📄 PDF Chat"], label_visibility="collapsed")
    st.markdown("---")
    st.markdown("**Courses**")
    for c in COURSES:
        st.markdown(f"• {c}")
...
# ── PDF CHAT MODE ──
    ...
    if uploaded:
        ...
        if st.button("Ask 💬"):
            if question:
                system = f"You are a study assistant. Answer questions ONLY based on this document content:\n\n{st.session_state.pdf_text}"
